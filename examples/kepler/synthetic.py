...
class DataGenerator(object):
    ...
    def __init__(
        ...
        if tsundowns is None:
            ...
            assert (
                tsundowns.shape[0] == params.nobs
            ), "tsundowns shape does not match number of observatories"
        ...
        if cadences is None:
            ...
            assert (
                cadences.shape[0] == params.nobs
            ), "cadences shape does not match number of observatories"
        ...
        if tstarts is None:
            ...
            assert (
                tstarts.shape[0] == params.nobs
            ), "tstarts shape does not match number of observatories"
        ...
        if tends is None:
            ...
            assert (
                tends.shape[0] == params.nobs
            ), "tends shape does not match number of observatories"
        self._tends = tends
    ...
    def generate_tobs(self):
        tobs = []
        for tstart, tend, sundown, cadence in zip(
